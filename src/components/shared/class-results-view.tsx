...
export function ClassResultsView({
  ...
  useEffect(() => {
    ...
    const termAssessments: string[] = [...(assessmentsByTerm[selectedTerm] || [])];
    if (selectedClassData?.gradeLevel === 'Standard 4') {
      termAssessments.push('Attainment');
    }
    setCurrentAssessments(termAssessments);
    if (!termAssessments.includes(selectedAssessment)) {
