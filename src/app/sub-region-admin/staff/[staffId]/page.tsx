...
import { ArrowLeft, Badge, BarChart, Loader2 } from 'lucide-react';
import { getTeacherTrackRecord } from './actions';
...
function ResultsTrackRecord({ staffId, subRegionId }: { staffId: string, subRegionId?: string }) {
    const [results, setResults] = useState<ResultRecord[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        getTeacherTrackRecord(staffId, subRegionId)
    ...
    if (loading) {
        return <div className="flex items-center gap-2 text-muted-foreground"><Loader2 className="animate-spin h-4 w-4"/> Loading results...</div>;
    }
    ...
    if (results.length === 0) {
        return <p className="text-muted-foreground">No end-of-term results found for this teacher.</p>;
    }
        ...
        <Table>
            <TableHeader>
                <TableRow>
                    ...
                    <TableHead className="text-right">Class Pass Rate</TableHead>
                </TableRow>
            </TableHeader>
            <TableBody>
                {results.map((r, i) => (
                    <TableRow key={i}>
                        ...
                        <TableCell className="text-right font-medium">{r.classPassRate.toFixed(1)}%</TableCell>
                    </TableRow>
                ))}
            </TableBody>
        </Table>
        ...
        {staff.role === 'teacher' && (
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        A summary of end-of-term pass rates for classes taught by this teacher. A pass is an overall average of 50% or higher.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                   <ResultsTrackRecord staffId={staff.uid} subRegionId={currentUser?.subRegionId} />
                </CardContent>
            </Card>
        )}
    </div>
