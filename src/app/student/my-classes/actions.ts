  ...
  try {
    ...
    if (classmateStudentIds.length > 0) {
       ...
       for (const chunk of studentChunks) {
           const schoolStudentsSnap = await db.collection('schools').doc(schoolId).collection('students').where(firestore.FieldPath.documentId(), 'in', chunk).get();
           classmates.push(...schoolStudentsSnap.docs.map(doc => {
               const data = doc.data();
               return {
                 id: doc.id,
        ...
        classmates = classmates
            .filter(student => student.uid !== uid) // Exclude the current user
            .sort((a, b) => a.fullName.localeCompare(b.fullName));
    }

    // Fetch Assignments
    const assignmentsQuery = db.collection('assignments').where('classId', '==', classId);
    ...
    assignments.sort((a, b) => a.title.localeCompare(a.title));
